...
Семь исходных миграций сохранены в `alembic/versions` как архивная линейная цепочка:
...
Архивная цепочка не создаёт исходную схему и падает на чистой БД уже при
обращении к таблице `chats`. Поэтому активный путь `alembic/current_versions`
содержит полную baseline-миграцию с тем же текущим revision
`5c0b937bc038`. Она создаёт текущую модель на пустой PostgreSQL и сохраняет
совместимость со штатно проштампованными существующими БД. Частично
применённые старые миграции требуют отдельного ручного аудита.

`alembic/env.py` импортирует метаданные всех моделей и выбирает
`ALEMBIC_DEV_URL` или `ALEMBIC_PROD_URL` по `ENVIRONMENT`; при отсутствии URL
команда завершается с явной ошибкой. `alembic.ini` URL не содержит.

Runtime-вызовы `create_all` удалены из main/socket/admin. Управление схемой
оставлено Alembic. Compose запускает одноразовый сервис `migrate` с
`alembic upgrade head` до старта приложений.

После миграций одноразовый Compose-сервис `seed` выполняет
`python -m scripts.seed_demo`. Идемпотентный seed доступен только при
`APP_ENV=demo`, использует вымышленные данные и локальные SVG-аватары. Smoke-
набор `tests/smoke` создаёт и удаляет отдельную БД с суффиксом `_test`,
применяет Alembic и не обращается к внешним сервисам.
...
3. Перед применением новых миграций к существующей production-БД сверить её
   фактическую схему и `alembic_version` с архивной историей.
