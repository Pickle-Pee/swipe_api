...
from config import SMS_CENTER_LOGIN, SMS_CENTER_PASSWORD
...
SMSC_LOGIN = f'{SMS_CENTER_LOGIN}'  # логин клиента
SMSC_PASSWORD = f'{SMS_CENTER_PASSWORD}'  # пароль
...
class SMSC(object):
    ...
    # DDMMYYhhmm, h1-h2, 0ts, +m) id - идентификатор сообщения. Представляет собой 32-битное число в диапазоне от 1 до
    # 2147483647. format - формат сообщения (0 - обычное sms, 1 - flash-sms, 2 - wap-push, 3 - hlr, 4 - bin,
    # 5 - bin-hex, 6 - ping-sms, 7 - mms, 8 - mail, 9 - call, 10 - viber, 11 - soc, 13 - telegram) sender - имя
    # отправителя (Sender ID). query - строка дополнительных параметров, добавляемая в URL-запрос (
    ...
    def _smsc_send_cmd(self, cmd, arg=""):
        ...
        _url = url
        arg = ifs(SMSC_LOGIN, "login=" + quote(SMSC_LOGIN) + "&psw=", "apikey=") + quote(
    ...
    def send_tg(self, tg_ids, message, query=""):

        params = f"login={SMSC_LOGIN}"
        params = f"psw={SMSC_PASSWORD}"
        params = "tg=1"
        params += "&phones=" + quote(tg_ids)
        params += "&mes=" + quote(message)

        if query:
            params += "&" + query

        m = self._smsc_send_cmd("send", params)

        if SMSC_DEBUG:
            if m[1] > "0":
                print(
                    f"Сообщение отправлено успешно. ID: {m[0]}, всего сообщений: {m[1]}, "
                    f"стоимость: {m[2]}, баланс: {m[3]}"
                )
            else:
                error_msg = f"Ошибка №{m[1][1:]}"
                if m[0] > "0":
                    error_msg += f", ID: {m[0]}"
                print(error_msg)
