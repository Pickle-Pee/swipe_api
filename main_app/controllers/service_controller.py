...
from fastapi import UploadFile, File, APIRouter, Depends, HTTPException, status, Request
from fastapi.responses import StreamingResponse, JSONResponse
from common.models import City, Region, User, UserPhoto
...
async def upload_profile_image(
    ...
    with SessionLocal() as db:
        ...
        try:
            ...
            # Логирование перед сохранением в базу
            print("Saving photo with the following parameters:")
...
#     if auth_header != f"Bearer {os.getenv('BOT_SECRET_TOKEN')}":
#         logger.error("Unauthorized attempt to update verification status")
#         raise HTTPException(status_code=403, detail="Unauthorized")
#
#     # Проверка chat_id
#     chat_id = request.json().get("chat_id")
#     if chat_id not in ALLOWED_CHAT_IDS:
#         logger.error(f"Unauthorized chat_id: {chat_id}")
