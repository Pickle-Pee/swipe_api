...
async def get_current_user(access_token: str = Depends(get_token)):
    with SessionLocal() as db:
        ...
        if not user_attributes:
            ...
            attributes_response = AttributesResponseUser(
                height=user_attributes.height,
                smoking_attitude=user_attributes.smoking_attitude,
                alcohol_attitude=user_attributes.alcohol_attitude,
                children_preference=user_attributes.children_preference,
                what_looking_for=user_attributes.what_looking_for,
                appearance=user_attributes.appearance,
                religion=user_attributes.religion
            )

        return PersonalUserDataResponse(
            id=user.id,
...
@router.get("/user/photos/{user_id}", response_model=UserPhotosResponse)
